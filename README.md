...
3. For each stable top-level PDF, the container copies it to a temporary work directory and runs NAPS2 deskewing.
4. The deskewed PDF is written atomically to `/mnt/gd/paperless-ngx/consume` as `/output`.
...
This container uses the following command shape for each input PDF:

```bash
naps2 console -i input.pdf -n 0 --deskew --disableocr ${NAPS2_EXTRA_ARGS} -o output.pdf
```

The important options are:

- `-i` imports the existing PDF.
...
Also verify host permissions on both mounted directories.
...
NAPS2 imports and re-exports the PDF while applying deskew. Existing PDFs with text layers may be rasterized or otherwise changed by NAPS2, so this container is mainly intended for scanned/image-only PDFs that will be OCRed by Paperless-ngx afterward.
